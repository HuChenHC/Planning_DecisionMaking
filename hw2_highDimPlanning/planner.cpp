...
{	
	...
	*planlength = 0;
	...
	mexPrintf("dof: %d\n", numofDOFs);
	// joint limits
	double angleMin[numofDOFs] = {0.0};
	double angleMax[numofDOFs];
	angleMax[0] = PI;
	...
	srand(time(0));
	// star & goal
	rrtNode* start = new rrtNode(armstart_anglesV_rad, numofDOFs);
	rrtNode* goal = new rrtNode(armgoal_anglesV_rad, numofDOFs);
	// initialize rrtPlanner
	rrtPlanner myPlanner(map, numofDOFs, x_size, y_size, start, goal, 0.3);
	
	// plan and return path
	vector<rrtNode*> path = myPlanner.planNow(angleMin, angleMax);  // from goal to start, backward
	int pathSize = path.size();
	*plan = (double**) malloc(pathSize*sizeof(double*));
    for (int i = 0; i < pathSize; i++){
		mexPrintf("\nplan[%d][]: ", i);
        (*plan)[i] = (double*) malloc(numofDOFs*sizeof(double)); 
        for(int j = 0; j < numofDOFs; j++){
			// i-th sample in path, dim j
            (*plan)[i][j] = path.at(pathSize-1-i)->getData().at(j);
			mexPrintf("%f  ", path.at(pathSize-1-i)->getData().at(j));
        }
    }
	*planlength = pathSize;
...
{	
	...
	*planlength = 0;
	...
	rrtNode* goal = new rrtNode(armgoal_anglesV_rad, numofDOFs);
	// initialize rrtPlanner
	rrtConnectPlanner myPlanner(map, numofDOFs, x_size, y_size, start, goal, 0.3);
	
	// plan and return path
	vector<rrtNode*> path = myPlanner.planRRTConnect(angleMin, angleMax);  // from goal to start, backward
	int pathSize = path.size();
	*plan = (double**) malloc(pathSize*sizeof(double*));
    for (int i = 0; i < pathSize; i++){
		mexPrintf("\nplan[%d][]: ", i);
...
{	
	...
	*planlength = 0;
	...
	rrtNode* goal = new rrtNode(armgoal_anglesV_rad, numofDOFs);
	// initialize rrtPlanner
	rrtStarPlanner myPlanner(map, numofDOFs, x_size, y_size, start, goal, 0.3);
	
	// plan and return path
	vector<rrtNode*> path = myPlanner.planRRTStar(angleMin, angleMax);  // from goal to start, backward
	int pathSize = path.size();
	*plan = (double**) malloc(pathSize*sizeof(double*));
    for (int i = 0; i < pathSize; i++){
		mexPrintf("\nplan[%d][]: ", i);
...
static void plannerPRM(
		...
		int* planlength)
{	
	// num of nodes grows by dof
	int iterNum = 10000*numofDOFs;
	...
	*planlength = 0;
	...
	srand(time(0));
	// initialize prmPlanner
	prmPlanner myPlanner(map, numofDOFs, x_size, y_size);
	// generate the roadMap
	myPlanner.geneRoadMap(iterNum, angleMin, angleMax);
	mexPrintf("sample size: %d, road map size: %d\n",iterNum, myPlanner.getRoadMapSize());
	// add star & goal
	prmNode* start = new prmNode(armstart_anglesV_rad, numofDOFs);
	prmNode* goal = new prmNode(armgoal_anglesV_rad, numofDOFs);
	myPlanner.addStartGoal(start, goal);
	
	// plan and return path
	vector<prmNode*> path = myPlanner.planAStar();  // from goal to start, backward
	int pathSize = path.size();
	*plan = (double**) malloc(pathSize*sizeof(double*));
    for (int i = 0; i < pathSize; i++){
		mexPrintf("\nplan[%d][]: ", i);
